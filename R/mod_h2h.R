...
mod_h2h_server <- function(id, carry_thru) {
  moduleServer(id, function(input, output, session) {
    ...
    df_h2h_prep <- reactive({
      req(carry_thru()$fty_parameters_met())

      df_h2h_today <- pluck(dfs_fty_roster, as.character(carry_thru()$selected$league_id)) |>
        ...
        bind_rows(
          filter(df_player_box_score, player_name %in% input$h2h_add_player) |>
            ...
            mutate(season = cur_season, competitor_id = as.numeric(input$h2h_competitor))
        ) |>
      ...
      df_h2h_future <- distinct(df_nba_schedule, game_date) |>
        ...
        select(any_of(colnames(pluck(dfs_h2h_past, as.character(carry_thru()$selected$league_id)))))

      # COMBINE
      df_h2h <- bind_rows(
        list(
          past = pluck(dfs_h2h_past, as.character(carry_thru()$selected$league_id)),
          today = df_h2h_today,
          future = df_h2h_future
        ),
        .id = "origin"
      ) |>
        left_join(
          select(pluck(df_rolling_stats, input$h2h_window), -c(espn_id, yahoo_id, player_name, team_slug)),
        ...
        df_h2h |>
          anti_join(
            filter(
              df_h2h,
              competitor_id == as.numeric(input$h2h_competitor),
              player_name %in% input$h2h_add_player,
              origin == "today"
            ),
          ...
          anti_join(
            filter(
              df_h2h,
              competitor_id == as.numeric(input$h2h_competitor),
              player_name %in% input$h2h_ex_player,
              origin == "future"
            ),
            by = join_by(competitor_id, player_id, game_date)
          ) |>
